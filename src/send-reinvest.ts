...
const claimScenario = async (vault: OpenedContract<MegaVault>, distributionPool: OpenedContract<MegaDistributionPool>, manager: Sender, address: string) => {
...
const reinvestScenario = async (vault: OpenedContract<MegaVault>, manager: Sender, address: string) => {
...
export const sendReinvest = async () => {
  ...
  for await (const vault of vaults) {
    const distributionPool = await vault.getDistributionPool().then(p => p.open());
