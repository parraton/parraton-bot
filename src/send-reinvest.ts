...
import {
  claimRewards,
  isClaimRewardsNeeded,
} from "./core/functions/claim-rewards";
...
export const sendReinvest = async () => {
  ...
  for (const vaultData of addresses.vaults) {
    ...
    if (!(await isClaimRewardsNeeded(distributionPool, vault.address))) {
      continue;
    }
    ...
    if (await isReinvestNeeded(vault.address)) {
      ...
      console.log(`No reinvest needed for ${vault.address}`);
    }
  }
};
